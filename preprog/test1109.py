...
# 高斯函数
    if median <= mean:
        ...
        gauss_down = (median / mean) * exp(-(((median - num) ** 2) / (2 * var ** 2)))
        u = ((gauss_up + gauss_down) / 2)**0.5
    elif median > mean:
        gauss_up = exp(-(((mean - num) ** 2) / (2 * var ** 2)))
        gauss_down = (mean / median) * exp(-(((median - num) ** 2) / (2 * var ** 2)))
...
def histogram_demo(image):
    plt.hist(image.ravel(), 255, [0, 255])#ravel函数功能是将多维数组降为一维数组
    plt.show()
...
if __name__ == '__main__':
    ...
    cv.imwrite('./result4.jpg', newimg)
    # histogram_demo(img),plt.title('im')
    # histogram_demo(newimg),plt.title('new')
    plt.subplot(221),plt.imshow(cv.cvtColor(img, cv.COLOR_BGR2RGB)),plt.title('img')
    plt.subplot(222),plt.imshow(cv.cvtColor(newimg, cv.COLOR_BGR2RGB)),plt.title('new')
    plt.subplot(223),plt.hist(img.ravel(), 255, [0, 255]),plt.title('imghist')
    plt.subplot(224),plt.hist(newimg.ravel(), 255, [0, 255]),plt.title('newhist')
    plt.show()
